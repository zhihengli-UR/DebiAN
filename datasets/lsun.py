# adapted from https://github.com/pytorch/vision/blob/main/torchvision/datasets/lsun.py
...
class LSUNClass(VisionDataset):
    def __init__(self, root, cache_root, transform=None, target_transform=None):
        import lmdb
        ...
        cache_fpath = os.path.join(cache_root, cache_file)
        if os.path.isfile(cache_fpath):
            self.keys = pickle.load(open(cache_fpath, "rb"))
        else:
            print(f'building LSUN cache: {cache_fpath}')
            with self.env.begin(write=False) as txn:
                self.keys = [key for key, _ in txn.cursor()]
            pickle.dump(self.keys, open(cache_fpath, "wb"))
            print(f'finished LSUN cache: {cache_fpath}')
